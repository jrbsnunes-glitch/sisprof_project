...
from .models import (Professor, EscolaNucleo, Escola, Cargo, Bairro, Serie, Motivo, 
                    AREA_ATUACAO_CHOICES, SITUACAO_FUNCIONAL_CHOICES)
from .forms import (ProfessorForm, EscolaNucleoForm, EscolaForm, CargoForm, BairroForm,
                   SerieForm, MotivoForm, LoginForm)
from django.db.models import Q, Count
from django.contrib.auth import login, logout
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from .utils.pdf_utils import (gerar_pdf_com_cabecalho, 
                                  obter_estilos_padrao, 
                                  obter_estilo_tabela_padrao)
"from .forms_usuarios import (UsuarioCreateForm, UsuarioUpdateForm, PerfilUsuarioForm, AlterarSenhaForm)"
...
def index(request):
    ...
    professores_com_area = Professor.objects.exclude(Q(area_atuacao='') | Q(area_atuacao__isnull=True)).count()
    ...
    ultimos_professores = Professor.objects.select_related(
        'escola_lotacao_nucleo', 'escola_nucleo', 'bairro'
    ...
    # Contar professores em escolas dependentes
    ...
    # Contar professores lotados direto em núcleos
...
def lista_professores(request):
    ...
    professores = Professor.objects.select_related(
        'escola_lotacao',                    # Escola de lotação
        'escola_lotacao__nucleo',            # Núcleo da escola
        'escola_lotacao__bairro',            # Bairro da escola ✓
        'escola_lotacao__nucleo__bairro',    # Bairro do núcleo (via escola) ✓
        'escola_nucleo',             # Núcleo direto
        'escola_nucleo__bairro',     # Bairro do núcleo direto ✓
        'bairro',                    # Bairro do professor
        'cargo',                     # Cargo do professor
        'serie',                     # Série
        'motivo',                    # Motivo
    ...
    if nucleo_id:
        professores = professores.filter(escola_lotacao__nucleo_id=nucleo_id)
    ...
    # Calcula estatísticas dos professores listados
    total_listado = professores.count()
    total_com_escola = professores.filter(escola_lotacao__isnull=False).count()
    total_com_area = professores.exclude(Q(area_atuacao='') | Q(area_atuacao__isnull=True)).count()
    total_com_matricula = professores.exclude(Q(matricula='') | Q(matricula__isnull=True)).count()
    
    # Para os filtros no template
    nucleos = EscolaNucleo.objects.all().order_by('nome')
    escolas = Escola.objects.select_related('nucleo').all().order_by('nucleo__nome', 'nome')
...
def carregar_escolas_por_nucleo(request):
    ...
    # Criar lista com o núcleo primeiro e depois as escolas dependentes
    data = []
    
    # Adicionar o próprio núcleo como primeira opção
    # Usar ID negativo ou string especial para diferenciar
    data.append({
        "id": nucleo.id,
    ...
    for e in escolas:
        data.append({
            "id": e.id,
            "nome": f"   └─ {e.nome}",  # Indentação visual
            "is_nucleo": False
        })
...
def buscar_bairros_ajax(request):
    ...
    # Busca bairros pelo nome, cidade ou estado
    bairros = Bairro.objects.filter(
        Q(nome__icontains=termo) | 
        Q(cidade__icontains=termo) |
        Q(estado__icontains=termo)
    ).values('id', 'nome', 'cidade', 'estado')[:10]  # Limita a 10 resultados
    ...
    data = [
        {
            ...
            'cidade': b['cidade'] or '',
            'estado': b['estado'] or '',
            'nome_completo': f"{b['nome']}" + (f" - {b['cidade']}/{b['estado']}" if b['cidade'] and b['estado'] else (f" - {b['cidade']}" if b['cidade'] else ""))
        }
        for b in bairros
    ]
...
def buscar_professores_ajax(request):
    ...
    # Busca professores pelo nome ou matrícula
    professores = Professor.objects.filter(
        Q(nome__icontains=termo) | Q(matricula__icontains=termo)
    ).values('id', 'nome', 'matricula')[:10]  # Limita a 10 resultados
...
# ═══════════════════════════════════════════════════════════════════════
# VIEWS PARA GERENCIAMENTO DE ESCOLAS
# Adicione estas views ao arquivo views.py
# ═══════════════════════════════════════════════════════════════════════
...
def lista_escolas_nucleo(request):
    ...
    escolas_com_contagem = []
    for escola in escolas:
        escola.num_dependentes = Escola.objects.filter(nucleo=escola).count()
        escola.num_professores = Professor.objects.filter(escola_lotacao_nucleo=escola).count()
        escolas_com_contagem.append(escola)
    
    # Paginação
    paginator = Paginator(escolas_com_contagem, 20)
...
def nova_escola_nucleo(request):
    ...
    return render(request, 'os_app/form_escola_nucleo.html', {'form': form, 'titulo': 'Cadastrar Escola Núcleo'})
...
def deletar_escola_nucleo(request, pk):
    ...
    # Verificar se tem escolas dependentes ou professores vinculados
    ...
    if request.method == 'POST':
        if num_dependentes > 0 or num_professores > 0:
            messages.error(request, f'Não é possível excluir "{escola.nome}" pois há {num_dependentes} escola(s) dependente(s) e {num_professores} professor(es) vinculado(s).')
            return redirect('os_app:lista_escolas_nucleo')
...
def lista_escolas_dependentes(request):
    ...
    for escola in escolas:
        escola.num_professores = Professor.objects.filter(escola=escola).count()
...
def nova_escola_dependente(request):
    ...
    return render(request, 'os_app/form_escola_dependente.html', {'form': form, 'titulo': 'Cadastrar Escola Dependente'})
...
def deletar_escola_dependente(request, pk):
    ...
    if request.method == 'POST':
        if num_professores > 0:
            messages.error(request, f'Não é possível excluir "{escola.nome}" pois há {num_professores} professor(es) vinculado(s).')
            return redirect('os_app:lista_escolas_dependentes')
...
"""
VIEWS DE RELATÓRIOS ATUALIZADAS - Sistema Flexível
Substitua as views de relatórios no arquivo os_app/views.py
"""
...
from django.db.models import Q, Count
from .models import (
    Professor, Escola, EscolaNucleo, Bairro, Cargo, Serie,
    AREA_ATUACAO_CHOICES, SITUACAO_FUNCIONAL_CHOICES, 
    MODALIDADE_CHOICES, TURNO_CHOICES
)
...
def relatorios_filtros(request):
    ...
    campos_disponiveis = [
        ...
        {'id': 'escola', 'nome': 'Escola', 'grupo': 'Lotação'},
...
def relatorios_resultado(request):
    ...
    if not campos_selecionados:
        ...
        campos_selecionados = ['id', 'nome', 'cargo', 'situacao_funcional', 'escola', 'area_atuacao']
    ...
    com_escola = professores.filter(
        Q(escola_lotacao__isnull=False) | Q(escola_lotacao_nucleo__isnull=False)
    ).count()
...
def relatorios_pdf(request):
    ...
    if ref_global:
        professores = professores.filter(ref_global__icontains=ref_global)
    if nucleo_id:
        professores = professores.filter(Q(escola_lotacao__nucleo_id=nucleo_id) | Q(escola_lotacao_nucleo_id=nucleo_id))
    if escola_id:
        professores = professores.filter(escola_lotacao_id=escola_id)
    if area:
        professores = professores.filter(area_atuacao=area)
...
def get_campo_valor_pdf(professor, campo):
    ...
    if campo == 'id':
        ...
        if professor.escola_lotacao:
            return professor.escola_lotacao.nome
        ...
        elif professor.escola_lotacao and professor.escola_lotacao.nucleo:
            return professor.escola_lotacao.nucleo.nome
        return '-'
    elif campo == 'endereco_escola':
        if professor.escola_lotacao:
            end = professor.escola_lotacao.endereco or ''
            num = professor.escola_lotacao.numero or ''
        ...
        if professor.escola_lotacao:
            ...
            return professor.escola_lotacao_nucleo.get_zona_display()
        return '-'
    elif campo == 'endereco':
...
def lista_usuarios(request):
    ...
    context = {
        ...
        'tipo_choices': PerfilUsuario.TIPO_USUARIO_CHOICES,
    }
