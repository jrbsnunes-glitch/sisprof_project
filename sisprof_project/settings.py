...
# Security Settings (em DEBUG=False, uso completo; em DEBUG=True, relaxado para desenvolvimento)
...
# Logging (em desenvolvimento usa pasta local; em produção pode usar /var/log/django/)
...
LOGGING = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': str(_log_dir / 'sisprof_error.log'),
        },
    },
    'loggers': {
        'django': {
...
# Se DB_USER não estiver no .env, usa SQLite para desenvolvimento local.
_db_user = config('DB_USER', default=None)
if _db_user is None or (isinstance(_db_user, str) and _db_user.strip() == ''):
    DATABASES = {
        'default': {
